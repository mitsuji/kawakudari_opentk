...
public class Std15
{
  ...
  {
    float x0 = (2.0f / screenW) * x - 1.0f;
    float y0 = 1.0f - (2.0f / screenH) * y;
    float x1 = x0 + (2.0f / screenW) * w;
    float y1 = y0 - (2.0f / screenH) * h;
  ...
  public void DrawChar (int cx, int cy, char c) {
    ulong glyph = ICHIGOJAM_FONT[c];
    for (int y = 0; y < CHAR_H; y++) {
      ulong line = (glyph >> ((CHAR_H-y-1)*CHAR_W)) & 0xff;
   ...
   *
   */
  private static readonly ulong [] ICHIGOJAM_FONT = {
    0x0000000000000000L,
    0xffffffffffffffffL,
    0xffaaff55ffaaff55L,
    0x55aa55aa55aa55aaL,
